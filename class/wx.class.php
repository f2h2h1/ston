...
$item_str    </Articles>
    ...
    */
    ...
    private function init_cache()
	{
        ...
        try // 创建连接
        {
            $dbh = new $dbclass($dataSourceName, $dataBaseConfig['dataBaseUserName'], $dataBaseConfig['dataBasePassWord'], array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8';"));
        }
    ...
    private function add_cache($cache_key,$cache_value)
    {
		...
		$expires_time = $this->timestamp+7000;
		$this->delete_cache($cache_key);
        $redis->lpush($cache_key, $expires_time);
        $redis->lpush($cache_key, $cache_value);
    }
